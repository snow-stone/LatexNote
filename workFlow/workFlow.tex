...
Downside for snappyHexMesh (they tell me snappy is really good at making cells orthogonal from the OpenFOAM forum of China) and T-Rex is that we lose the absolute control of every cell.
...
\item Pointwise
\begin{enumerate}
\item Estimate wall unity $\frac{\nu}{u_\tau}$ for first mesh size 
\item Examine domains' aspect ratio see if it's symmetric 
\item Select all connectors and apply equal spacing 
\item Re-examine 1 
\item Still not symmetric, cut the other half then mirror and merge. Apply spacing constraint to wall. 
\item Check for mesh quality (pointwise has different metrics to OpenFOAM): aspect ratio, non-orthogonality, skewness... 
...
\begin{enumerate}
\item Rescale to meter : \textit{transformPoints -scale '(0.001 0.001 0.001)'} 
...
\item Double check the solver used in sbatch job submission file. Add memory test(one-noed version or multi-noeds version) if needed. 
\item Check \textit{\$nos} \textit{\$logFile} numbering. Name the submission file using \textbf{8 chars}. 
\item Final check for fluid properties. $\nu$ or non-newtonian laws. \\
\end{enumerate}
...
\item python based postprocessing using paraview (auto-determine machine zaurak/visu )
...
\item Check for fluctuations at Dai's mesurement using \textit{probes\_Dai} 
...
Try : \\
\begin{itemize}
\item[$\diamond$] $\nabla p \cdot \vec{n}=0$ at all patches and add a reference pressure value somewhere in the tube to leave $p_{outlet}=10$ which may be too tight.

\item[$\diamond$] pipe flow with \textit{cyclic} BC
...
4. Using classic results inside the boundary layer mean profile along with the external flow with $u_\infty$, calculate the thickness $\theta$ then estimate $c_f^{(1)}$ with eq. (3.13). \textcolor{blue}{lois de parois classique Cf=[2log ...] Is this what you mean Alex?}\\
